...
export default function User() {
    ...
    useEffect(() => {
      const cancelBooking = async () => {
        try {
          const response = await axios.put('http://localhost:8080/api/user/cancelBooking', {
            email: userDetails.email,
            password: cancelPassword,
            bookingID: selectedBooking.bookingId,
          });
  
          // Handle the cancellation response, update UI accordingly
          console.log('Booking cancellation response:', response.data);
          setSelectedBooking.status(INACTIVE);
          // Reset the cancelConfirmation state
  ...
  useEffect(() => {
    const fetchUserBookings = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/api/user/userBooking/${userDetails.email}`);
        ...
        <ul style={{ listStyleType: 'none', padding: 0 }}>
          {bookings.map((booking, index) => (
            <li key={index} style={{ textDecoration: "none" }}>
              <div onClick={() => handleBookingClick(booking)} style={{ display: "flex" }}>
                <div style={{ display: "flex", flexDirection: "column" }}>
                  <div>{booking.schedule.source}</div>
                  <div>{booking.schedule.pickupTime}</div>
                </div>------{calculateDuration(booking.schedule.pickupTime, booking.schedule.arrivalTime)}------<div style={{ display: "flex", flexDirection: "column" }}>
...
<p>
  <strong>Booking Id:</strong> {selectedBooking.bookingId.substring(0,8)}
</p>
<p>
  <strong>Booking status:</strong> {selectedBooking.status.substring(7)}
</p>
<p>
  <strong>Source:</strong> {selectedBooking.schedule.source}
</p>
<p>
  <strong>Destination:</strong> {selectedBooking.schedule.destination}
</p>
<p>
  <strong>Flight:</strong> {selectedBooking.schedule.flightName}
</p>
<p>
  <strong>Travel Date:</strong> {formatDate(selectedBooking.schedule.travelDate)}
</p>
<p>
  <strong>Pickup Time:</strong> {selectedBooking.schedule.pickupTime}
</p>
<p>
  <strong>Arrival Time:</strong> {selectedBooking.schedule.arrivalTime}
</p>
</fieldset>
<fieldset>
<legend><h2>Passenger Details</h2></legend>
...
<p>
  <strong>Payment Id:</strong> {selectedBooking.paymentInfo.paymentId.substring(0,8)}
</p>
<p>
  <strong>Account No:</strong> {selectedBooking.paymentInfo.accountNo}
</p>
<p>
  <strong>Total Amount:</strong> {selectedBooking.paymentInfo.totalAmount}
</p>
</fieldset>
            </div>
