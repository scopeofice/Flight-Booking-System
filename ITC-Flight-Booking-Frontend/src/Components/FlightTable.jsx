  ...
  const handleDelete = async (flightName) => {
    try {
      const response = await axios.delete(
        `http://localhost:8080/api/flights/deleteFlight/${flightName}`
      );
      console.log(response.data);
      setDel(true);
    } catch (error) {
  ...
  const handleEdit = async () => {
    try {
      const response = await axios.put(
        "http://localhost:8080/api/flights/updateFlight",
        selectedFlight
      );
      console.log(response.data);
      setShow(true);
          ...
          <form className="form" style={{ margin: "20px" }}>
            <>
              ...
              <input
                ...
                value={
                  editMode ? selectedFlight.airlineName : newFlight.airlineName
                }
              ...
              <input
                ...
                value={
                  editMode ? selectedFlight.flightName : newFlight.flightName
                }
                onChange={editMode ? handleEditChange : handleChange}
              />
              <input
              ...
              <button
                type="button"
                onClick={editMode ? handleEdit : handleNewFlight}
              >
                {editMode ? "Update" : "Add"}
              </button>
              ...
              {flights.map((flight) => (
                <tr key={flight.flightName}>
                  ...
                  <td>
                    ...
                    <button style={{marginLeft:"5px"}} onClick={() => handleDelete(flight.flightName)}>
                      Delete
                    </button>
                  </td>
                </tr>
